...
const ReservationModal: React.FC<ReservationModalProps> = ({ isOpen, onClose, language, onContactConcierge, initialStep = 'type' }) => {
  ...
  const [globalMethod, setGlobalMethod] = useState<'stripe' | 'paypal' | 'usdt'>('stripe');
        ...
        <div className="flex flex-col gap-6">
          ...
          <div className="bg-white/5 p-6 rounded-xl border border-white/10 relative overflow-hidden">
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => setGlobalMethod('stripe')}
                  className={`text-[10px] uppercase tracking-widest px-3 py-1 rounded-full border transition-all ${globalMethod === 'stripe' ? 'border-luxury-gold text-luxury-gold bg-luxury-gold/10' : 'border-white/10 text-slate-500 hover:text-white'}`}
                >
                  Credit Card
                </button>
                {/*
            ...
            {globalMethod === 'stripe' ? (
              <StripePayment
                amount={stayType === 'overnight' ? 1200 : 450}
                onSuccess={() => setStep('confirmed')}
                onError={(err) => console.error(err)}
              />
            ) : globalMethod === 'paypal' ? null /* (
