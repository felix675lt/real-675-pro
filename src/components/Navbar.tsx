...
const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <nav
      className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-luxury-900/90 backdrop-blur-md py-4 border-b border-white/5' : 'bg-transparent py-6'
        }`}
    >
              ...
              <div className="absolute right-0 top-full mt-2 w-32 bg-luxury-900 border border-white/10 rounded-lg shadow-xl overflow-hidden animate-fade-in">
                {languages.map((lang) => (
                  <button
                    ...
                    className={`block w-full text-left px-4 py-3 text-xs uppercase tracking-widest hover:bg-white/10 transition-colors ${language === lang.code ? 'text-luxury-gold' : 'text-slate-300'
                      }`}
                  >
                    {lang.label}
                  </button>
